package prg.provide.galitt.invoice;
...
static String concate=".";
 static CellStyle style;
    ...
    @Test
    @Parameters("Name")
	public static void Galitt(String Name) throws IOException {
		...
		try {
		 ...
		 for (int i = 1; i <= excelData.size(); i++) {
				...
				UtilInvoice.editNonPoInvoice(driver,wait,excelData.get(i).get("InvoiceNo"),excelData.get(i).get("PR Type"),excelData.get(i).get("SupplierCode"),excelData.get(i).get("InvoiceDate"),excelData.get(i).get("InvoiceAmt"),excelData.get(i).get("TaxAmt"),excelData.get(i).get("Approver"));
				...
				test.log(Status.PASS, "Logout for Invoice Validation");
